...
def load_percolation() -> tuple[dict[str, Any], np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    top_sizes = np.fromfile("top_sizes.bin", dtype=np.uint32).reshape(total_states, 3)
...
def animate_percolation(save_path=None, interval=50, dpi=100):
    ...
    ax_grid.hlines(grid_lines, -0.5, L - 0.5, color="#e0e0e0", linewidth=0.5, alpha=0.4, zorder=0)
    ax_grid.vlines(grid_lines, -0.5, L - 0.5, color="#e0e0e0", linewidth=0.5, alpha=0.4, zorder=0)
    ...
    ax_grid.plot(static_x, static_y, "o", markersize=marker_size, color="#d0d0d0", alpha=0.3)

    horizontal_mask = bonds["direction"] == 1
    bond_segments = np.empty((len(bonds), 2, 2))
    
    bond_segments[horizontal_mask] = np.array([
        [(c, r), (c + 1, r)] for r, c in zip(bonds["row"][horizontal_mask], bonds["col"][horizontal_mask])
    ])
    bond_segments[~horizontal_mask] = np.array([
        [(c, r), (c, r + 1)] for r, c in zip(bonds["row"][~horizontal_mask], bonds["col"][~horizontal_mask])
    ])
    ...
    bond_collection = mcoll.LineCollection(
        ...
        color="#1B5299",
        alpha=0.3,
        linewidth=max(0.25, marker_size / 2),
        zorder=1
    )
    ...
    cluster_scatter = ax_grid.scatter(
        coords[:, 0], coords[:, 1],
        s=0, zorder=2, alpha=0.8
    )

    step_text = ax_grid.text(
        0.02, 1.02, "Initial Grid",
    ...
    n_bins = 256
    custom_cmap = LinearSegmentedColormap.from_list("cluster_sizes", colors[1:], N=n_bins)
    unconnected_color = colors[0]

    def init():
        bond_collection.set_segments([])
        cluster_scatter.set_sizes(np.full(L * L, base_node_size))
        cluster_scatter.set_facecolors(np.full((L * L, 4), [*plt.matplotlib.colors.to_rgb(unconnected_color), 0.5]))
        for line in lines:
            line.set_data([], [])
    ...
    def update(frame):
        ...
        all_colors = np.full((L * L, 4), [*plt.matplotlib.colors.to_rgb(unconnected_color), 0.5])
        ...
        if np.any(connected_mask):
            ...
            color_scale = np.power(size_ratios, 0.7)  # Adjusted power for better color distribution
